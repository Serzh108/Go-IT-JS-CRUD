...
function updatePost(e) {
  if (e.target.nodeName === 'BUTTON' && e.target.dataset.action === 'update') {
    if (services.refs.Form.querySelector('button[data-action="update"]'))
      return;
    // 1. При нажатии на кнопку update поста добавить кнопку Update post в форму
    // (и создать ей дата атрибут c id поста который мы обновляем)
